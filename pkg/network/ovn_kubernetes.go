...
func renderOVNKubernetes(conf *operv1.NetworkSpec, bootstrapResult *bootstrap.BootstrapResult, manifestDir string,
	...
	if slices.Contains(featureGates.KnownFeatures(), apifeatures.FeatureGateNoOverlayMode) && featureGates.Enabled(apifeatures.FeatureGateNoOverlayMode) {
		...
		if c.BGPManagedConfig.BGPTopology != "" {
			...
			klog.V(5).Infof("BGP managed configuration enabled for no-overlay mode")
