...
func TestRenderNetworkNodeIdentity(t *testing.T) {
	...
	testTLSArgRendering(t, "webhook ovnkube-identity", func(t *testing.T, tlsProfile bootstrap.TLSProfile) string {
		networkConfig, bootstrapResult, client := setupTest(t)
		// Merge the TLS settings from the parameter into the full bootstrapResult
	...
	t.Run("HyperShift enabled", func(t *testing.T) {
		...
		t.Run("should successfully render the network-node-identity Deployment", func(t *testing.T) {
			deployment := mustFindRenderedObj(t, assertRenderSuccess(t, networkConfig, bootstrapResult, client),
				"Deployment", "network-node-identity", &appsv1.Deployment{})
		...
		testTLSArgRendering(t, "webhook ovnkube-identity", func(t *testing.T, tlsProfile bootstrap.TLSProfile) string {
			// Merge the TLS settings from the parameter into the HyperShift bootstrapResult
			bootstrapResult.TLSProfile = tlsProfile
			deployment := mustFindRenderedObj(t, assertRenderSuccess(t, networkConfig, bootstrapResult, client),
				"Deployment", "network-node-identity", &appsv1.Deployment{})
...
func testTLSArgRendering(t *testing.T, name string, getCommandStr func(*testing.T, bootstrap.TLSProfile) string) {
	g := NewWithT(t)

	t.Run("when TLS profile adherence is LegacyAdheringComponentsOnly", func(t *testing.T) {
		t.Run(fmt.Sprintf("should render the %s command without the TLS CLI args", name), func(t *testing.T) {
...
func mustFindRenderedObj[T any](t *testing.T, objs []*uns.Unstructured, kind, name string, objType T) T {
	...
	err := runtime.DefaultUnstructuredConverter.FromUnstructured(objs[index].Object, objType)
	g.Expect(err).NotTo(HaveOccurred())
...
func findOvnkubeIdentityExec(t *testing.T, cmdArgs []string) string {
	...
	startIdx := strings.Index(cmdArgs[2], "exec /usr/bin/ovnkube-identity")
	g.Expect(startIdx).NotTo(Equal(-1), "Could not find 'exec /usr/bin/ovnkube-identity' in command args: %q", cmdArgs[2])
