	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectManagedEnabled {
				...
				g.Expect(ovnkubeConf).To(ContainSubstring("frr-namespace="+frrK8sNamespace),
					"ConfigMap frr-namespace should match the frrK8sNamespace constant %q", frrK8sNamespace)

				// Verify that the frr-k8s namespace manifest defines a Namespace
				// with the expected frrK8sNamespace name
				nsBytes, err := os.ReadFile(filepath.Join(manifestDirOvn, "network/frr-k8s/000-ns.yaml"))
				g.Expect(err).NotTo(HaveOccurred())
				nsObj := &uns.Unstructured{}
				g.Expect(yaml.Unmarshal(nsBytes, &nsObj.Object)).NotTo(HaveOccurred())
				g.Expect(nsObj.GetKind()).To(Equal("Namespace"))
				g.Expect(nsObj.GetName()).To(Equal(frrK8sNamespace),
					"frr-k8s namespace manifest should define Namespace %q", frrK8sNamespace)
